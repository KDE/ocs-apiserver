...
class Application_Model_DbTable_PploadFiles extends Local_Model_Table
{
    ...
    private function fetchAllFiles($collection_id, $ignore_status = true, $activeFiles = false, $perpage = 1000,
                                   $page = 1) {
    ...
    public function getFiles(      $originId = null, $status = 'active', $clientId = null, $ownerId = null,
                                   $collectionId = null, $collectionStatus = 'active', $collectionCategory = null,
                                   $collectionTags = null, $collectionContentId = null, $types = null, $category = null,
                                   $tags = null, $ocsCompatibility = 'all', $contentId = null, $search = null,
                                   $ids = null, array $favoriteIds = null, $downloadedTimeperiodBegin = null,
                                   $downloadedTimeperiodEnd = null, $sort = 'name', $perpage = 20, $page = 1) {
        $prefix = 'ppload_';
        $name = 'ppload.files';
        ...
        if ($collectionTags !== null && $collectionTags !== '') {
            foreach (explode(',', $collectionTags) as $tag) {
                $tag = trim($tag);
                if ($tag) {
                    $where[] = "({$prefix}collections.tags = " . $this->getDb()
                                                                      ->quote($tag) . " OR {$prefix}collections.tags LIKE " . $this->getDb()
                                                                                                                                   ->quote("$tag,%") . " OR {$prefix}collections.tags LIKE " . $this->getDb()
                                                                                                                                                                                                    ->quote("%,$tag,%") . " OR {$prefix}collections.tags LIKE " . $this->getDb()
                                                                                                                                                                                                                                                                       ->quote("%,$tag") . ')';
                }
        ...
        if ($tags !== null && $tags !== '') {
            foreach (explode(',', $tags) as $tag) {
                $tag = trim($tag);
                if ($tag) {
                    $where[] = "({$prefix}files.tags = " . $this->getDb()
                                                                ->quote($tag) . " OR {$prefix}files.tags LIKE " . $this->getDb()
                                                                                                                       ->quote("$tag,%") . " OR {$prefix}files.tags LIKE " . $this->getDb()
                                                                                                                                                                                  ->quote("%,$tag,%") . " OR {$prefix}files.tags LIKE " . $this->getDb()
                                                                                                                                                                                                                                               ->quote("%,$tag") . ')';
                }
        ...
        if (!empty($favoriteIds['ownerIds']) || !empty($favoriteIds['collectionIds']) || !empty($favoriteIds['fileIds'])) {
            $where[] = $this->_convertFavoriteIdsToStatement($favoriteIds, array(
        ...
        if ($downloadedTimeperiodBegin || $downloadedTimeperiodEnd) {
            ...
            $_columns = str_replace("{$prefix}files.downloaded_count AS downloaded_timeperiod_count",
                                    'downloaded_timeperiod.count AS downloaded_timeperiod_count', $this->_columns);
            ...
            $files = $this->fetchRowset($_join . ' ' . $statementOption . " ORDER BY $order LIMIT $perpage OFFSET $offset",
                                        $values);
            ...
            $pagination = Flooer_Utility_Pagination::paginate($this->count($_join . ' ' . $statementOption, $values),
                                                              $perpage, $page);
            ...
            $files = $this->fetchRowset($this->_join . ' ' . $statementOption . " ORDER BY $order LIMIT $perpage OFFSET $offset",
                                        $values);
            $this->setColumns($columns);
            ...
            $this->setColumns($this->_columns);
            $pagination = Flooer_Utility_Pagination::paginate($this->count($this->_join . ' ' . $statementOption,
                                                                           $values), $perpage, $page);
            $this->setColumns($columns);
        }
