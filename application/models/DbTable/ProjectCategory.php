...
class Application_Model_DbTable_ProjectCategory extends Local_Model_Table
{
     ...
     */
    public function fetchTree($isActive = false, $withRoot = true, $depth = null) {
     ...
     * @internal param bool $clearCache
     */
    public function fetchTreeWithParentId($isActive = true, $depth = null) {
        $sqlActive = $isActive == true ? " parent_active = 1 AND pc.is_active = 1" : '';
        $sqlDepth = is_null($depth) == true ? '' : " AND depth <= " . (int)$depth;
        $sqlHaving = $sqlActive || $sqlDepth ? "HAVING {$sqlActive} {$sqlDepth}" : '';
        $sql = "
    ...
    public function moveElement($node, $newLeftPosition) {
        ...
        try {
            ...
            $this->_db->query("UPDATE {$this->_name} SET rgt = rgt + :space WHERE rgt >= :newLeftPosition;", array(
                'space'           => $space,
                'newLeftPosition' => $newLeftPosition
            ));
            $this->_db->query("UPDATE {$this->_name} SET lft = lft + :space WHERE lft >= :newLeftPosition;", array(
                'space'           => $space,
                'newLeftPosition' => $newLeftPosition
            ));
            ...
            // remove old space
            $this->_db->query("UPDATE {$this->_name} SET rgt = rgt - :space WHERE rgt > :srcPosition;", array(
                'space'       => $space,
                'srcPosition' => $srcPosition
            ));
            $this->_db->query("UPDATE {$this->_name} SET lft = lft - :space WHERE lft >= :srcPosition;", array(
                'space'       => $space,
                'srcPosition' => $srcPosition
            ));
    ...
    public function fetchCategoriesForForm($valueCatId) {
        $level = 0;
        $ancestors = array("catLevel-{$level}" => $this->fetchMainCatForSelect(Application_Model_DbTable_ProjectCategory::ORDERED_TITLE));
        $level++;
